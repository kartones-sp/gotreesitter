...
func (p *Parser) buildReduceChildren(entries []stackEntry, start, end, childCount int, productionID uint16, arena *nodeArena) ([]*Node, []FieldID) {
	...
	rawFieldIDs, hasInherited := p.buildFieldIDs(childCount, productionID, arena)
	hasFields := rawFieldIDs != nil
	...
	if !hasFields && hasInherited {
		for i := start; i < end; i++ {
			n := entries[i].node
			if n != nil && !n.isExtra && n.fieldIDs != nil {
				if idx := int(n.symbol); idx < len(symbolMeta) && !symbolMeta[n.symbol].Visible {
					hasFields = true
					break
				}
			}
